...
| Runner fleet + gRPC | **Partial** | gRPC server on 9091 with optional TLS/mTLS; Helm `runnerGrpcTls` + optional `certManager.runnerGrpc` Certificate; Vault PKI issues runner client certs on registration (`pravah.runner.pki.*`); `RunnerGrpcIdentityInterceptor` SPIFFE SAN auth; local `vault-pki-bootstrap.sh` |
...
| Vault Transit tenant secrets (ADR-007) | Partial | `HttpVaultTransitClient`; pipeline-service `transit` provider stores ciphertext in Postgres; write-only `value` on create/update; decrypt at execution (`SecretTransitIT`) |
