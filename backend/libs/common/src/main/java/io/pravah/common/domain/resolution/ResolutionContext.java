 ...
 *
 * <p>This is an immutable record except for the {@code secretsCache} which is intentionally mutable
 * to allow caching resolved secrets within a single execution context.
 *
...
public record ResolutionContext(
  ...
  public static ResolutionContext forValidation(UUID tenantId) {
    return new ResolutionContext(tenantId, null, null, Map.of(), new ConcurrentHashMap<>());
  }

  /** Creates a context for execution start (variables resolved, secrets deferred). */
  public static ResolutionContext forExecution(
    ...
    return new ResolutionContext(
        tenantId, executionId, executionTime, variableContext, new ConcurrentHashMap<>());
  }
}
