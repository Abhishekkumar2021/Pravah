...
public interface RunnerRepository extends JpaRepository<Runner, UUID> {
    ...
    List<Runner> findByTenantIdAndStatusIn(@Param("tenantId") UUID tenantId,
                                           @Param("statuses") List<RunnerStatus> statuses);

    @Query("SELECT r FROM Runner r WHERE r.tenantId = :tenantId " +
    ...
    @Query("SELECT r FROM Runner r WHERE r.status IN ('ONLINE', 'BUSY') " +
