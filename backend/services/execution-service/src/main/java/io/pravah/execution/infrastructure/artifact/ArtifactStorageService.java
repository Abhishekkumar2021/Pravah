...
/**
 * Service for managing artifacts in S3/MinIO storage. Handles upload, download, presigned URL
 * generation, and lifecycle management.
 *
 * <p>Bucket layout: pravah-artifacts/ └── tenants/{tenant_id}/ └── executions/{execution_id}/ └──
 * jobs/{job_id}/ ├── output/ │ ├── data.parquet │ └── data.json └── logs/ └── execution.log.gz
 *
...
public class ArtifactStorageService {
  ...
  private void ensureBucketExists() {
    try {
      ...
      log.warn(
          "Could not check/create bucket '{}': {}. Artifact storage may not be available.",
          props.bucket(),
          e.getMessage());
    }
  }
  ...
  public String buildKey(
      String tenantId, UUID executionId, UUID jobId, ArtifactType type, String filename) {
    return String.format(
        "tenants/%s/executions/%s/jobs/%s/%s/%s",
        tenantId, executionId, jobId, type.getFolder(), filename);
  }

  /**
   * Generate a presigned URL for uploading an artifact. Runners use this to upload directly to
  ...
  public PresignedUrlResponse generateUploadUrl(
    ...
    log.debug(
        "Generated upload URL for artifact: key={}, expires={}", key, presignedUrl.expiration());
  ...
  public PresignedUrlResponse generateDownloadUrl(
    ...
    var getRequest = GetObjectRequest.builder().bucket(props.bucket()).key(key).build();
    ...
    return new PresignedUrlResponse(
        presignedUrl.url().toString(), key, presignedUrl.expiration(), 0);
  }
  ...
  public ArtifactMetadata upload(
    ...
    return new ArtifactMetadata(key, filename, type, contentLength, contentType, Instant.now());
  }
  ...
  public ArtifactMetadata upload(
    ...
    return new ArtifactMetadata(key, filename, type, content.length, contentType, Instant.now());
  }
  ...
  public List<ArtifactMetadata> listJobArtifacts(String tenantId, UUID executionId, UUID jobId) {
    String prefix =
        String.format("tenants/%s/executions/%s/jobs/%s/", tenantId, executionId, jobId);

    var listRequest = ListObjectsV2Request.builder().bucket(props.bucket()).prefix(prefix).build();
        ...
        .map(
            obj -> {
              ...
              return new ArtifactMetadata(
                  key, filename, type, obj.size(), null, obj.lastModified());
            })
        .toList();
  }
