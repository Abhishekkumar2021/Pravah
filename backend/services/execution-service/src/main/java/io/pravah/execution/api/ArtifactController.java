 ...
 *
 * <p>Artifacts are stored in MinIO (S3-compatible) and accessed via presigned URLs. Runners
 * upload/download directly to MinIO; this API only generates URLs.
 *
...
public class ArtifactController {
  ...
  public ResponseEntity<PresignedUrlResponse> generateUploadUrl(
    ...
    var response =
        artifactStorageService.generateUploadUrl(
            requireTenantId(),
            request.executionId(),
            request.jobId(),
            request.type(),
  ...
  public record GenerateUploadUrlRequest(
      UUID executionId, UUID jobId, ArtifactType type, String filename, String contentType) {}

  public record GenerateDownloadUrlRequest(
      UUID executionId, UUID jobId, ArtifactType type, String filename) {}
}
