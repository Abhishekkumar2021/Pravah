 ...
 *
 * <p>Runs daily to delete artifacts from completed executions older than
...
public class ArtifactCleanupJob {
    ...
    public void cleanupExpiredArtifacts() {
        log.info("Starting artifact cleanup job",
                kv("retention_days", props.retentionDays()));
        ...
        try {
            ...
            log.info("Found {} executions with artifacts to clean",
                    kv("count", expiredExecutions.size()));
            ...
            for (var execution : expiredExecutions) {
                try {
                    ...
                    log.warn("Failed to clean artifacts for execution",
                            kv("execution_id", execution.getId()),
                            kv("error", e.getMessage()));
                }
            }

            log.info("Artifact cleanup completed",
