...
public class ConnectionController {
    ...
    public ResponseEntity<ConnectionDto> getConnection(@PathVariable UUID connectionId) {
        UUID tenantId = requireTenantId();
        return connectionService.get(tenantId, connectionId)
                .map(this::toDto)
    ...
    @PostMapping
    public ResponseEntity<ConnectionDto> createConnection(@RequestBody CreateConnectionRequest request) {
        UUID tenantId = requireTenantId();
        Connection connection = connectionService.create(tenantId, new ConnectionService.CreateConnectionRequest(
                request.name(),
                request.description(),
                request.connectorId(),
                request.config()
    ...
    public ResponseEntity<ConnectionDto> updateConnection(
        ...
        UUID tenantId = requireTenantId();
        Connection connection = connectionService.update(tenantId, connectionId,
    ...
    public record TestConfigRequest(
            String connectorId,
            Map<String, Object> config
    ) {}
