...
/**
 * Base class for JDBC-based database connectors.
 * Provides common functionality for PostgreSQL, MySQL, SQL Server, Oracle, etc.
 */
public abstract class AbstractJdbcConnector implements SourceConnector, SinkConnector {
    ...
    private List<FieldInfo> discoverFields(DatabaseMetaData meta, String catalog,
    ...
    private List<String> discoverPrimaryKeys(DatabaseMetaData meta, String catalog,
                                             String schema, String table) throws SQLException {
    ...
    @Override
    public WriteResult write(Map<String, Object> config, String streamName,
                             List<Record> records, WriteOptions options) {
        if (records.isEmpty()) {
    ...
    private void setParameters(PreparedStatement stmt, List<String> columns,
    ...
    private static class JdbcRecordIterator implements RecordIterator {
        ...
        JdbcRecordIterator(Connection conn, PreparedStatement stmt,
                           ResultSet rs, List<String> columnNames) throws SQLException {
            this.conn = conn;
            this.stmt = stmt;
            this.rs = rs;
