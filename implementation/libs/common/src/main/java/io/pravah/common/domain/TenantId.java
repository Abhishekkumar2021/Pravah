 ...
 * <p>
 * Tenant IDs must be lowercase alphanumeric with hyphens, 3-63 characters.
 * This format ensures compatibility with DNS naming (for subdomains)
 * and PostgreSQL schema naming.
 *
...
public record TenantId(
    ...
    @Pattern(
        regexp = PATTERN,
        message = "Tenant ID must be lowercase alphanumeric with hyphens, 3-63 characters, starting with a letter"
    )
    String value
) {
    /**
     * Regex pattern for validating tenant IDs.
     * Lowercase alphanumeric with hyphens, 3-63 characters, must start with a letter and end with alphanumeric.
     */
